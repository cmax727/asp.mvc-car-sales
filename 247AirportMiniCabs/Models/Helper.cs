...
namespace AlinTuriCab.Models
{
    public class Helper
    {
        ...
        private List<Country> liListItem;
        ...
        [Display(Name = "ddCountries")]
        public List<Country> CountryList
        {
            get
            {
                bool boHitFirstZeroPriorityCountry = false;
                liListItem = new List<Country>();
                Country country = new Country();
                //country.Name = "";
                //country.Code = "-1";
                liListItem.Add(country);

                // populate the country drop down
                for (int i = 0; i < Helper.ISOCountryData.Count; i++)
                {
                    if (i > 0 &&
                        !boHitFirstZeroPriorityCountry &&
                        Helper.ISOCountryData[i].ListPriority == 0)
                    {
                    ...
                    country = new Country();
                    country.Name = Helper.ISOCountryData[i].ISOCode.ToString();
                    country.Code = Helper.ISOCountryData[i].CountryName;
                    liListItem.Add(country);
                }
        ...
        public List<Month> GetMonths
        {
            get
            {
                ...
                for (int i = 1; i <= 12; i++)
                {
                    ...
                    if (i < 10)
                    {
                        month.MonthName = "0" + i.ToString();
                        month.MonthValue = "0" + i.ToString();
                    }
                    else
                    {
                        month.MonthName = i.ToString();
                        month.MonthValue = i.ToString();
                    }
        ...
        public List<StartDateYear> GetStartDateYear
        {
            get
            {
                List<StartDateYear> years = new List<StartDateYear>();
                StartDateYear year;
                for (int i = 0; i < 10; i++)
                {
                    year = new StartDateYear();

                    year.Year = (DateTime.Now.Year - i).ToString();
                    year.YearValue = (DateTime.Now.Year - i).ToString().Substring(2);
                    years.Add(year);
                }
        ...
        public List<ExpiryDateYear> GetExpiryDateYear
        {
            get
            {
                List<ExpiryDateYear> years = new List<ExpiryDateYear>();
                ExpiryDateYear year;
                for (int i = 0; i < 10; i++)
                {
                    year = new ExpiryDateYear();
